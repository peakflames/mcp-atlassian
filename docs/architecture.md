...
- `issue.py` — `JiraIssue` with null-filtering in `to_simplified_dict()`
...
- Development: `jira_get_issue_development_info`, `jira_get_issues_development_info`
- Metrics: (internal, used by SLA / analytics)
...
- Changelog: (internal, via batch operations)
...
- (Analytics, spaces, etc. available via related mixins)
...
- If caller requests `fields='*all'`, all non-null fields returned
