...
This document describes the operational concept for four source-level fixes in `peakflames/mcp-atlassian`: two JIRA field-completeness defects (`jira/constants.py`, `jira/issues.py`, `jira/search.py`) and two multi-instance OAuth defects (`utils/oauth.py`). These fixes were identified during a downstream AI-assistant integration discovery process in the sibling repo `the downstream MCP server`, which deferred five TOR requirements here because they require Python source changes rather than Dockerfile/config changes.
...
| `jira_get_issue(fields='*all')` null handling | Even after the sentinel fix, `models/jira/issue.py` `to_simplified_dict()` emits all stored custom fields including unset ones as `{"value": null}` dicts. The final `v is not None` filter does not remove these because the wrapper dict is not `None`. On Jira instances with thousands of custom field definitions, a single `*all` call floods LLM context with thousands of null entries. |
...
Together, these changes mean a downstream caller like the downstream project's `the downstream MCP server` gets contract-correct `*all` responses (populated fields only), and multi-instance users get deterministic site resolution with an immediate, actionable failure mode instead of silent wrong-site behavior discovered later.
...
|---|---|
| Downstream MCP operator (e.g. `the downstream MCP server` maintainer) | Does `fields='*all'` actually return all fields now? Do I still need to maintain a local patched field list? |
...
**Steps:**
1. Client calls `jira_get_issue(issue_key="PROJ-123", fields="*all")`
...
**Actor:** LLM assistant (e.g., a downstream AI-assistant integration via `the downstream MCP server`)
**Trigger:** User asks "which other tickets are linked to PROJ-456?"
...
4. LLM calls `jira_get_issue(issue_key="PROJ-456", fields="summary,status,issuelinks")`
...
| Downstream dependency | `the downstream MCP server` must bump its `FROM peakflames/mcp-atlassian:` tag to pick up these fixes before its 5 deferred TOR IDs can be verified closed |
...
| `ATLASSIAN_OAUTH_CLOUD_ID` | Environment variable naming the specific Cloud site a deployment should use; already consumed downstream in `the downstream MCP server`'s Dockerfile |
| TOR ID | Test Objective Requirement identifier from the project's peak-workflow requirements baseline; this project resolves TOR-01-MDNHg8G, TOR-01-JKVYLpG, TOR-03-J54hatP, and TOR-03-quJs6mG from `the downstream MCP server`, and unblocks TOR-03-XnkkkuJ which depends on all four |
