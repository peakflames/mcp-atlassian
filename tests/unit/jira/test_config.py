...
def test_from_env_projects_readonly_and_blocked():
    ...
    with patch.dict(
        os.environ,
        {
            ...
            "JIRA_USERNAME": "u",
            "JIRA_API_TOKEN": "t",
            "JIRA_PROJECTS_READONLY": "PROJA, legacy ",
            "JIRA_PROJECTS_BLOCKED": "PROJB,SECRET",
        },
        clear=True,
    ):
        config = JiraConfig.from_env()
        assert config.projects_readonly == "PROJA, legacy "
        assert config.projects_blocked == "PROJB,SECRET"
...
def test_projects_readonly_set_normalises():
    ...
    config = JiraConfig(
        ...
        username="u",
        api_token="t",
        projects_readonly=" proja , legacy ",
    )
    assert config.projects_readonly_set == frozenset({"PROJA", "LEGACY"})
...
def test_projects_blocked_set_normalises():
    ...
    assert config.projects_blocked_set == frozenset({"PROJB", "SECRET"})
